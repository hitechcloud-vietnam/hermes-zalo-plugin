...
class TestLoadConfig(unittest.TestCase):
    _OK_ENV = {"HITECHCLOUD_LANDING_BRIDGE_URL": "https://mcp.hitechcloud.vn/tools/landing_upload_image",
               "HITECHCLOUD_LANDING_BRIDGE_KEY": "k"}

    def test_requires_https(self):
        with self.assertRaises(BridgeError):
            load_bridge_config({"HITECHCLOUD_LANDING_BRIDGE_URL": "http://x/y", "HITECHCLOUD_LANDING_BRIDGE_KEY": "k"}, "/c")
    ...
    def test_max_dim_env_override(self):
        cfg = load_bridge_config({**self._OK_ENV, "HITECHCLOUD_LANDING_IMAGE_MAX_DIM": "1600"}, "/c")
        self.assertEqual(cfg.max_dim, 1600)

    def test_max_dim_bad_values_fall_back(self):
        for bad in ("banana", "0", "-5", "99999"):
            cfg = load_bridge_config({**self._OK_ENV, "HITECHCLOUD_LANDING_IMAGE_MAX_DIM": bad}, "/c")
            self.assertEqual(cfg.max_dim, 1024, bad)

    def test_rejects_userinfo(self):
        with self.assertRaises(BridgeError):
            load_bridge_config({"HITECHCLOUD_LANDING_BRIDGE_URL": "https://u:p@x/y", "HITECHCLOUD_LANDING_BRIDGE_KEY": "k"}, "/c")

    def test_requires_key(self):
        with self.assertRaises(BridgeError):
            load_bridge_config({"HITECHCLOUD_LANDING_BRIDGE_URL": "https://x/y"}, "/c")

    def test_ok(self):
        cfg = load_bridge_config({"HITECHCLOUD_LANDING_BRIDGE_URL": "https://mcp.hitechcloud.vn/tools/landing_upload_image", "HITECHCLOUD_LANDING_BRIDGE_KEY": "k"}, "/c")
...
class TestUploadRecent(unittest.TestCase):
    ...
    def _bridge(self, recents, resolver_result, post_result):
        cfg = BridgeConfig(url="https://mcp.hitechcloud.vn/tools/landing_upload_image", key="secret-key", cache_dir=self.tmp)
    ...
    def test_happy_path_returns_compact_and_hides_bytes(self):
        ...
        b = self._bridge(
            ...
            {"image_url": "https://landingpage.hitechcloud.vn/my-slug/assets/hero-abc.jpg"},
        )
    ...
    def test_rejects_non_durable_url(self):
        ...
        b = self._bridge(
            [_Rec(p)],
            {"chat_id": "c", "conv_id": "c"},
            {"image_url": "https://mcp.hitechcloud.vn/media/tmp123.jpg"},  # temporary transport
        )
        with self.assertRaises(BridgeError):
            b.upload_recent(task_id="s", slug="s")
...
class TestUploadResizesRealImages(unittest.TestCase):
    ...
    def _bridge(self, recents, max_dim=1024):
        cfg = BridgeConfig(url="https://mcp.hitechcloud.vn/tools/landing_upload_image",
