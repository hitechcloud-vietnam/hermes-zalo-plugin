...
class TestValidateUrl(unittest.TestCase):
    def test_https_mcp_ok(self):
        self.assertEqual(
            validate_image_url("https://mcp.hitechcloud.vn/media/abc.png", DEFAULT_ALLOWED_HOSTS),
            "mcp.hitechcloud.vn",
        )

    def test_reject_http(self):
        with self.assertRaises(ImageUrlError):
            validate_image_url("http://mcp.hitechcloud.vn/media/abc.png", DEFAULT_ALLOWED_HOSTS)
...
class TestFetch(unittest.TestCase):
    def test_happy_png(self):
        data, ext = fetch_image_from_url(
            "https://mcp.hitechcloud.vn/media/abc.png", http_open=_opener(PNG)
        )
    ...
    def test_happy_jpeg(self):
        data, ext = fetch_image_from_url(
            "https://mcp.hitechcloud.vn/media/abc.jpg", http_open=_opener(JPEG)
        )
    ...
    def test_reject_non_image_body(self):
        with self.assertRaises(ImageUrlError):
            fetch_image_from_url(
                "https://mcp.hitechcloud.vn/media/x.png", http_open=_opener(NOT_IMAGE)
            )
    ...
    def test_reject_oversize_streamed(self):
        ...
        with self.assertRaises(ImageUrlError):
            fetch_image_from_url(
                "https://mcp.hitechcloud.vn/media/big.png",
    ...
    def test_reject_empty(self):
        with self.assertRaises(ImageUrlError):
            fetch_image_from_url(
                "https://mcp.hitechcloud.vn/media/empty.png", http_open=_opener(b"")
            )
    ...
    def test_redirect_error_propagates(self):
        with self.assertRaises(ImageUrlError):
            fetch_image_from_url(
                "https://mcp.hitechcloud.vn/media/x.png",
...
class TestDefaultOpenerRedirectBlock(unittest.TestCase):
    ...
    def test_no_redirect_handler_raises_on_3xx(self):
        ...
        req = urllib.request.Request("https://mcp.hitechcloud.vn/media/x.png")
        with self.assertRaises(ImageUrlError):
    ...
    def test_default_opener_wires_no_redirect_handler(self):
        ...
        try:
            try:
                _default_http_open("https://mcp.hitechcloud.vn/media/x.png", 0.001)
            except OSError:
                pass
        finally:
...
class TestResolveAllowedHosts(unittest.TestCase):
    ...
    def test_csv_parsed(self):
        self.assertEqual(
            resolve_allowed_hosts("mcp.hitechcloud.vn, cdn.hitechcloud.vn"),
            ("mcp.hitechcloud.vn", "cdn.hitechcloud.vn"),
        )
